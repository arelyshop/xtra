...
function initApp() {
    ...
    function renderCart() {
        ...
        if (cart.length === 0) {
            // AQUÍ ESTÁN LOS CAMBIOS APLICADOS: SVG gris suave y Botón gris
            container.innerHTML = `<div class="flex flex-col items-center justify-center h-full text-gray-400 mt-10">
                <svg width="60" height="60" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="mb-4 text-gray-300">
                ...
                <a href="colecciones.html" class="mt-4 bg-gray-200 text-gray-800 px-6 py-2 rounded font-bold text-sm hover:bg-gray-300 transition-colors">Ver Catálogo</a>
            </div>`;
        } else {
            let html = '';
            cart.forEach(item => {
